  ...
  return (
    <div className="min-h-screen bg-ink-950 flex items-center justify-center px-6 relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        <div className="absolute top-1/3 left-1/4 w-[500px] h-[500px] bg-cinnabar-600/6 rounded-full blur-[120px]" />
        <div className="absolute bottom-1/4 right-1/3 w-[400px] h-[400px] bg-imperial-500/5 rounded-full blur-[100px]" />
        {["学", "习", "中", "文", "你", "好"].map((c, i) => (
          <motion.span key={i} className="absolute font-chinese text-ink-800/20 select-none pointer-events-none" style={{ fontSize: `${35 + i * 8}px`, left: `${5 + i * 16}%`, top: `${10 + ((i * 23) % 70)}%` }} animate={{ y: [0, -15, 0], opacity: [0.1, 0.2, 0.1] }} transition={{ duration: 4 + i * 0.4, repeat: Infinity, ease: "easeInOut", delay: i * 0.2 }}>{c}</motion.span>
        ))}
      </div>
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }} className="relative w-full max-w-md">
        ...
        <div className="rounded-2xl border border-ink-700/30 bg-ink-900/60 backdrop-blur-xl p-8">
          <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="text-2xl font-bold text-white text-center mb-2">{t.auth.welcomeBack}</motion.h1>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-sm text-ink-400 text-center mb-8">{t.auth.continueJourney}</motion.p>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <motion.button type="submit" disabled={loading} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="w-full py-3.5 rounded-xl bg-gradient-to-r from-cinnabar-600 to-cinnabar-500 text-white font-semibold flex items-center justify-center gap-2 hover:shadow-lg hover:shadow-cinnabar-500/20 transition-shadow disabled:opacity-50">
              {loading ? <motion.span animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full" /> : <>{t.auth.signIn}<ArrowRightIcon size={16} /></>}
            </motion.button>
          </form>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="text-sm text-ink-400 text-center mt-6">
            {t.auth.noAccount}{" "}<Link to="/register" className="text-cinnabar-400 hover:text-cinnabar-300 font-medium transition-colors">{t.auth.startLearning}</Link>
          </motion.p>
        </div>
      </motion.div>
    </div>
